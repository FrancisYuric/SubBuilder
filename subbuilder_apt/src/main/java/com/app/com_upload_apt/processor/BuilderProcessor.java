...
//        //创建类空壳
            ...
            while (!temp.getQualifiedName().toString().equals(Object.class.getName())) {
                mAbstractProcessor.mMessenger.printMessage(Diagnostic.Kind.WARNING, "ciruy message:"+ elementAnnotatedWithBuilderClass.getQualifiedName()+","+temp.getQualifiedName().toString()+","+Object.class.toString());
            ...
            typeSpecBuilder
                    ...
                    .addMethod(MethodSpec.methodBuilder("build").addModifiers(Modifier.PUBLIC).addParameter(originClassName, "p1")
    ...
    private void analyseEnclosedElements(String generatedClassName, String packageName, TypeSpec.Builder typeSpecBuilder, Element declaredType, AnnotationProcessor mAbstractProcessor) {
        mAbstractProcessor.mMessenger.printMessage(Diagnostic.Kind.WARNING,
                "ciruy method warning:" +
                        declaredType.getSimpleName()+" begin");
        for (ExecutableElement executableElement : ElementFilter.methodsIn(declaredType.getEnclosedElements())) {
//            mAbstractProcessor.mMessenger.printMessage(Diagnostic.Kind.WARNING,"ciruy method warning:"+declaredType.getSimpleName()+","+executableElement.getSimpleName());
            ...
            for (int i = 0; i < parameters.size(); i++) {
                Modifier[] modifiers = Arrays.copyOf(parameters.get(i).getModifiers().toArray(), parameters.get(i).getModifiers().size(), Modifier[].class);
